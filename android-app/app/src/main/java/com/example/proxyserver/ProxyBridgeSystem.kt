...
class ProxyBridgeSystem(
    ...
    private inner class ProxyHttpServer(hostname: String, port: Int) : NanoHTTPD(hostname, port) {
        ...
        private fun handleResponse(
            ...
            when (val headerMessage = messageQueue.dequeue(config.defaultTimeoutMs)) {
                ...
                is ProxyMessage.ResponseHeaders -> {
                    return if (shouldStreamResponse(headerMessage)) {
                        streamResponse(messageQueue, headerMessage, releaseQueue)
                    } else {
                        bufferResponse(messageQueue, headerMessage, releaseQueue)
                    }
                }
        ...
        private fun shouldStreamResponse(headerMessage: ProxyMessage.ResponseHeaders): Boolean {
            val hasChunkedEncoding = headerMessage.headers.entries.any { (key, values) ->
                key.equals("transfer-encoding", ignoreCase = true) &&
                    values.any { it.contains("chunked", ignoreCase = true) }
            }
            ...
            val contentTypeValues = headerMessage.headers.entries
                ...
                ?.value
            val contentType = contentTypeValues?.firstOrNull()?.lowercase(Locale.ROOT) ?: ""

            return contentType.contains("text/event-stream")
        }

        private fun bufferResponse(
            messageQueue: MessageQueue,
            ...
            return try {
                while (true) {
                    when (val message = messageQueue.dequeue(config.defaultTimeoutMs)) {
                        ...
                        ProxyMessage.StreamEnd -> {
                            ...
                            return buildFixedLengthResponse(status, headerMessage.headers, bodyBytes)
                        }

                        is ProxyMessage.Error -> {
                            val statusCode = message.status ?: headerMessage.status
                            return createErrorResponse(lookupStatus(statusCode), message.message)
                        }
                ...
                logger.warn("响应等待超时: ${timeout.message}")
                createErrorResponse(Response.Status.REQUEST_TIMEOUT, "响应超时")
            } catch (closed: QueueClosedException) {
                logger.warn("响应队列已关闭: ${closed.message}")
                createErrorResponse(Response.Status.INTERNAL_ERROR, "连接已关闭")
            } finally {
                releaseQueue()
            }
        ...
        private fun buildFixedLengthResponse(
            ...
            val response = newFixedLengthResponse(
                status,
                normalizeContentType(headers.entries
                    .firstOrNull { it.key.equals("content-type", ignoreCase = true) }
                    ?.value
                    ?.firstOrNull()),
            ...
            headers.forEach { (key, values) ->
                ...
                values.forEach { value ->
                    if (key.equals("content-length", ignoreCase = true)) {
                        response.addHeader(key, bodyBytes.size.toString())
                    } else {
