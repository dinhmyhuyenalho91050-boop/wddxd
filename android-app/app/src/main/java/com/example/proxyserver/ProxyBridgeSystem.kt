...
class ProxyBridgeSystem(
    ...
    private inner class ProxyHttpServer(hostname: String, port: Int) : NanoHTTPD(hostname, port) {
        ...
        override fun serve(session: IHTTPSession): Response {
            ...
            var managedByStream = false

            return try {
                val proxyRequest = buildProxyRequest(session, requestId)
                forwardRequest(proxyRequest)
                val result = handleResponse(messageQueue, requestId)
                managedByStream = result.streaming
                result.response
        ...
        private data class ResponseResult(val response: Response, val streaming: Boolean)

        private fun handleResponse(messageQueue: MessageQueue, requestId: String): ResponseResult {
            when (val headerMessage = messageQueue.dequeue(config.defaultTimeoutMs)) {
                is ProxyMessage.Error -> {
                    ...
                    return ResponseResult(createErrorResponse(status, headerMessage.message), false)
                }

                is ProxyMessage.ResponseHeaders -> {
                    return ResponseResult(streamResponse(messageQueue, headerMessage, requestId), true)
                }

                else -> {
                    return ResponseResult(
                        createErrorResponse(Response.Status.INTERNAL_ERROR, "无效的响应类型"),
                        false
                    )
                }
            }
