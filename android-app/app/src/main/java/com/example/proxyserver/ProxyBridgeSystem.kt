...
class ProxyBridgeSystem(
    ...
    private inner class ProxyHttpServer(hostname: String, port: Int) : NanoHTTPD(hostname, port) {
        ...
        private fun normalizeRequestBody(bodyBytes: ByteArray, contentTypeHeader: String?): String {
            ...
            return when {
                contentType.contains("application/json") -> bodyBytes.toString(Charsets.UTF_8)
                contentType.contains("application/x-www-form-urlencoded") -> encodeFormBody(bodyBytes)
                contentType.startsWith("text/") || contentType.contains("javascript") || contentType.contains("xml") ->
                    bodyBytes.toString(Charsets.UTF_8)
                else -> bufferString(bodyBytes)
            }
        }

        private fun encodeFormBody(bodyBytes: ByteArray): String {
            val bodyText = bodyBytes.toString(Charsets.UTF_8)
            if (bodyText.isEmpty()) {
                return ""
            }

            val orderedValues = LinkedHashMap<String, Any>()
            ...
            bodyText.split("&").forEach { pair ->
                ...
                val isArrayKey = decodedKey.endsWith("[]")
                val key = if (isArrayKey) decodedKey.dropLast(2) else decodedKey

                val existing = orderedValues[key]
                when {
                    existing == null && !isArrayKey -> orderedValues[key] = value
                    existing == null && isArrayKey -> orderedValues[key] = mutableListOf(value)
                    existing is MutableList<*> -> (existing as MutableList<String>).add(value)
                    else -> {
                        val list = mutableListOf(existing as String, value)
                        orderedValues[key] = list
                    }
                }
            }

            return buildJsonString(orderedValues)
        }

        private fun buildJsonString(values: LinkedHashMap<String, Any>): String {
            ...
            when (value) {
                ...
                is Iterable<*> -> {
                    ...
                    value.forEachIndexed { index, entry ->
                        ...
                        builder.append('"').append(escapeJson(entry as String)).append('"')
                    }
                    builder.append(']')
                }
            ...
            input.forEach { ch ->
                when (ch) {
                    ...
                    '\t' -> result.append("\\t")
                        ...
                        if (ch < ' ') {
                            result.append(String.format(Locale.ROOT, "\\u%04x", ch.code))
                        } else {
                            result.append(ch)
                        }
                    }
